    ...
    void vzyMarcheSTP() {
        try (DataInputStream s = new DataInputStream(
            ...
            while (true){
                ...
                System.out.printf("| %-6s | %-9s | %6s | %33s | %45s | %8s | %7s |%n",
                        "OACI", "Indicatif", "Immat.", "Modèle", "Position", "Alt.", "Vit.");
                for (ObservableAircraftState state : stateSet) {
                    ...
                    System.out.printf("| %-6s | %-9s | %6s | %33s | %45s | %8s | %7s |%n",
                            ...
                            state.getPosition().latitude(),state.getPosition().longitude(),
                            Math.round(state.getAltitude() * 100.) / 100.,
                            Math.round(Units.convertTo( state.getVelocity(), Units.Speed.KILOMETER_PER_HOUR)));
                }
                Thread.sleep(200);
